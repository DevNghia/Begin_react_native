...
const TableHeight = ({navigation}) => {
  const studentId = useSelector(state => state.data.data._id);
  const {data, isLoading} = useQuery('useGetTableHeight', () =>
    FetchApi.getTableHeight(studentId),
  );
    ...
    <View style={styles.container}>
      <ScrollView>
        <View
          ...
          <Text style={{color: 'black', fontSize: 20}}>Chiều cao</Text>
        ...
        {(data._data.growth_info || []).map((item, index) => {
          return (
            <View style={styles.blockList} key={index}>
              ...
              <Text style={{color: 'black', fontSize: 14, marginVertical: 5}}>
                Chiều cao: {item._height}
              </Text>
              <Text style={{color: 'black', fontSize: 14, marginVertical: 5}}>
                Danh giá: {item._index_height}
              </Text>
            </View>
          );
        })}
